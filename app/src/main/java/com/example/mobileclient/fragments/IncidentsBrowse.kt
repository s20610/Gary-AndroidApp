    ...
    override fun onCreateView(
        ...
        accidentReportViewModel.getAccidentReportsResponse.observe(viewLifecycleOwner) { response ->
            if (response.isSuccessful) {
                var accidentReports: List<AccidentReport> = response.body()!!
                accidentReports.forEach { accidentReport ->
                    accidentReport.emergencyType =
        ...
        binding.listView.setOnItemClickListener { adapterView, view, i, l ->
            accidentReportViewModel.pickedAccidentReport =
                adapterView.getItemAtPosition(i) as AccidentReport
